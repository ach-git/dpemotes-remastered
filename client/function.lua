...
Citizen.CreateThread(function()
    TriggerEvent('chat:addSuggestion', '/e', 'Faire une animations', {{ name="emotename", help="Faire une dance, un geste..."}})
    TriggerEvent('chat:addSuggestion', '/emote', 'Faire une animations', {{ name="emotename", help="Faire une dance, un geste..."}})
end)
...
function EmotesOnCommand(source, args, raw)
  local EmotesCommand = ""
  for a in pairsByKeys(hEmote.Emotes) do
...
function EmoteMenuStart(args, hard)
    ...
    if etype == "dances" then
        if hEmote.Dances[name] ~= nil then
          if OnEmotePlay(hEmote.Dances[name]) then end
        end
    elseif etype == "props" then
        if hEmote.PropEmotes[name] ~= nil then
          if OnEmotePlay(hEmote.PropEmotes[name]) then end
        end
    elseif etype == "emotes" then
        if hEmote.Emotes[name] ~= nil then
          if OnEmotePlay(hEmote.Emotes[name]) then end
        else
          if name ~= "🕺 Dance Emotes" then end
        end
    elseif etype == "expression" then
        if hEmote.Expressions[name] ~= nil then
          if OnEmotePlay(hEmote.Expressions[name]) then end
        end
    end
end
...
function EmoteCommandStart(source, args, raw)
    ...
    if hEmote.Emotes[name] ~= nil then
      if OnEmotePlay(hEmote.Emotes[name]) then end return
    elseif hEmote.Dances[name] ~= nil then
      if OnEmotePlay(hEmote.Dances[name]) then end return
    elseif hEmote.PropEmotes[name] ~= nil then
      if OnEmotePlay(hEmote.PropEmotes[name]) then end return
    else
